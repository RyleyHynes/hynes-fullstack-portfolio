...
export default function PhotoCarousel({ photos, index, onPrev, onNext, onUpload }: PhotoCarouselProps) {
  const current = photos[index]
  const total = photos.length
  const src = typeof current === 'string' ? current : current ? URL.createObjectURL(current) : ''
  return (
